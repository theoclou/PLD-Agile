...
const TextSidebar = React.memo(({ deliveryData, warehouse, sections, onDelete, highlightedDeliveryId, onMouseEnterDelivery, onMouseLeaveDelivery }) => {
  ...
  const courierColors = {
    0: "#FF0000", 1: "#0000FF", 2: "#00FF00", 3: "#FFA500",
    4: "#800080", 5: "#FF1493", 6: "#00FFFF", 7: "#FFD700",
    8: "#463941", 9: "#b95e1f"
  };
    ...
    Object.keys(grouped).forEach(courierId => {
      if (courierId !== 'unassigned' && Array.isArray(grouped[courierId])) {
        grouped[courierId].sort((a, b) => {
          const timeA = a.arrivalTime ? a.arrivalTime.hours * 60 + a.arrivalTime.minutes : Infinity;
          const timeB = b.arrivalTime ? b.arrivalTime.hours * 60 + b.arrivalTime.minutes : Infinity;
          return timeA - timeB;
        });
      }
    });
                    ...
                    return limitedSections.length > 0 ? (
                      limitedSections.map((section, index) => (
                        <div key={index} className="section-item">
                          ...
                          <span className="section-name">{section.name || "Undefined"}</span>
                        </div>
                      ))
                    ) : (
  ...
  const renderDeliveryPoint = (delivery, courierId = null) => {
    const relatedSections = sections.filter(
      section => section.origin.id === delivery.deliveryAdress.id.toString() ||
        ...
        >
          ...
          <div className="warehouse-section">
            <div>
              ...
              <span className="section-info" style={{ color: courierId ? courierColors[courierId] : 'inherit' }}>
          ...
          <div className="button-container">
            <button onClick={() => onDelete(delivery.deliveryAdress.id)} className="deletebutton">
              &times;
            </button>
          </div>
        </div>
    ...
    <div className="textual-sidebar">
      ...
      {groupedDeliveries.unassigned?.length > 0 && (
        <div>
          ...
          {groupedDeliveries.unassigned.map(delivery => renderDeliveryPoint(delivery))}
        </div>
      )}
        ...
        .map(([courierId, deliveries]) => (
          <div key={courierId}>
            <h2 className="section-title" style={{ color: courierColors[courierId] }}>
              ...
              <button onClick={() => toggleDeliveryInfo(courierId)} className="toggle-button">
                {expandedDeliveries[courierId] ? '⏶' : '⏷'}
              </button>
            </h2>
            {expandedDeliveries[courierId] && deliveries.map(delivery => renderDeliveryPoint(delivery, courierId))}
          </div>
        ))}
    </div>
  );
